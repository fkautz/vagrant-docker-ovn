...
CONTAINER1_NAME=$(sudo docker inspect --format '{{ .Name }}' $CONTAINER1_ID)
...
CONTAINER2_NAME=$(sudo docker inspect --format '{{ .Name }}' $CONTAINER2_ID)
...
nsenter --target $CONTAINER1_PID --net ip link set eth0 up
...
ovn-nbctl lport-set-addresses $CONTAINER1_NAME $CONTAINER1_MAC_ADDR
...
ovn-nbctl lport-set-addresses $CONTAINER2_NAME $CONTAINER2_MAC_ADDR
